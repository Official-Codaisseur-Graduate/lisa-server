...
app.intent("user_info", async (conv, params, permissionGranted) => {
  if (permissionGranted) {
    ...
    if (requestedPermission === "DEVICE_PRECISE_LOCATION") {
      conv.user.storage.location = await getLocation(
        conv.device.location.coordinates
      );
      await readMenu(conv);
    }
  } else {
    return conv.close(
      "Zonder locatie kan ik niet weten aan" +
...
app.intent("delete_location", conv => {
  ...
  conv.close("Oké, ik zal de volgende keer " + "de locatie opnieuw opvragen");
});

app.catch((conv, error) => {
  console.error(error);
  conv.close(
    "Er ging iets mis. " + "Waarschijnlijk is er geen verbinding met de server"
  );
});
...
readMenu = async conv => {
  ...
  const date =
    body.date && body.date.startDateTime
      ? new Date(body.date.startDateTime)
      : body.date
      ? new Date(body.date)
      : new Date();
  ...
  if (!menuExists) {
    return conv.ask(
      `Het menu voor die dag bij ${name} ` +
  ...
  switch (body.type) {
    case "Voorgerecht":
      conv.ask(await voorgerechtSentence(id, date)); //body.type
      break;
    case "Nagerecht":
      conv.ask(await nagerechtSentence(id, date));
      break;
    case "Hoofdgerecht":
...
getLocation = async coordinates => {
  ...
  const { latitude, longitude } = findNearest(coordinates, vitalisLocations);
